...
## 2024-05-25 - [Run independent MongoDB queries concurrently]
**Learning:** When a Python endpoint makes multiple independent asynchronous I/O-bound calls (like fetching various aggregate statistics in `session_management_api.py`), running them sequentially adds unnecessary latency.
**Action:** Wrap independent Motor queries in `asyncio.gather()` to execute them concurrently and decrease overall response time.
