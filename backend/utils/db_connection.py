...
def _pyodbc():
    """
    Resolve the active pyodbc module at call time.

    Some tests replace `sys.modules["pyodbc"]`; resolving lazily avoids stale
    references captured at import time.
    """
...
class SQLServerConnectionBuilder:
    ...
    def get_available_driver(cls) -> Optional[str]:
        ...
        try:
            installed_drivers = [driver for driver in _pyodbc().drivers()]
    ...
    def create_optimized_connection(
        ...
        conn = _pyodbc().connect(conn_str, timeout=timeout)
    ...
    @staticmethod
    def is_connection_valid(conn) -> bool:
        """
