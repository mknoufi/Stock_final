...
def test_clear_service_logs_async_offload():
    """Test that log clearing is offloaded to a thread to prevent blocking"""
    with (
        patch("backend.api.admin_control_api.Path.exists", return_value=True),
        patch("backend.api.admin_control_api.asyncio.to_thread") as mock_to_thread,
        patch("builtins.open", mock_open()) as mock_file
    ):
        # Setup mock_to_thread to execute the function immediately (asynchronously)
        async def execute_in_thread(func, *args, **kwargs):
            return func(*args, **kwargs)

        mock_to_thread.side_effect = execute_in_thread

        response = client.post("/api/admin/control/logs/clear", params={"service": "backend"})

        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True

        # Verify it was offloaded to a thread
        assert mock_to_thread.called

        # Verify the file operation actually happened
        mock_file.assert_called_once()
        # Note: The exact path argument depends on default settings, usually "app.log"
        # We check mode and encoding primarily
