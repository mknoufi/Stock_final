...
def _apply_update(document: dict[str, Any], update: dict[str, Any]) -> bool:
    """Apply $set and $inc updates to the document."""
    modified = False
...
class InMemoryCollection:
    ...
    async def create_index(self, keys: Any, **kwargs: Any) -> str:
        """Mock create_index."""
        # In a real DB this would create an index. Here we just return a dummy name.
        return "index_name"
    ...
    def aggregate(self, pipeline: list[dict[str, Any]]) -> InMemoryCursor:
        """mimic basic aggregation pipeline."""
        results = [copy.deepcopy(doc) for doc in self._documents]

        for stage in pipeline:
            if "$match" in stage:
                results = [doc for doc in results if _match_filter(doc, stage["$match"])]
            elif "$group" in stage:
                # Very basic grouping
                group_id = stage["$group"]["_id"]
                # Only support $sum: 1 for now as that's what we use

                groups = {}
                for doc in results:
                    key = (
                        doc.get(group_id.lstrip("$"))
                ...
                sort_rule = list(stage["$sort"].items())[0]
                key, direction = sort_rule
                reverse = direction < 0
...
class InMemoryDatabase:
    ...
    def __init__(self):
        ...
        self.auth_otps = InMemoryCollection()
        self.auth_reset_tokens = InMemoryCollection()
        ...
        self.stock_snapshots = InMemoryCollection()  # Added explicitly
    ...
    def __getattr__(self, name: str) -> InMemoryCollection:
        """Allow accessing collections via db.name. Fallback to __getitem__."""
        return self[name]
...
def _setup_core_services(monkeypatch, fake_db, server_module) -> None:
    ...
    manager = _NoOpMigrationManager()
    monkeypatch.setattr(lifespan_module, "migration_manager", manager, raising=False)
    monkeypatch.setattr(g, "migration_manager", manager, raising=False)
    monkeypatch.setenv("RATE_LIMIT_ENABLED", "false")
...
def _setup_mock_services(monkeypatch, server_module) -> None:
    ...
    monkeypatch.setattr(lifespan_module, "sql_connector", mock_sql, raising=False)
    ...
    monkeypatch.setattr(lifespan_module, "database_health_service", mock_health, raising=False)
    ...
    if hasattr(lifespan_module, "auto_sync_manager"):
        monkeypatch.setattr(lifespan_module, "auto_sync_manager", mock_auto_sync, raising=False)
    ...
    monkeypatch.setattr(
        lifespan_module, "scheduled_export_service", mock_export_service, raising=False
    )
