...
def test_either_methods():
    ...
    assert r.fold(lambda l: "left", lambda r: "right") == "right"
    
    # Left
    l = Either.left("err")
    assert l.is_left
    assert not l.is_right
    assert l.map(lambda x: x).is_left
    assert l.map_left(lambda x: x + "!")._value == "err!"
    assert l.get_or_else("default") == "default"
    assert l.fold(lambda l: "left", lambda r: "right") == "left"
...
def test_auth_utils_production_init():
    ...
    with patch('backend.config.settings.JWT_SECRET', 'secret'), \
