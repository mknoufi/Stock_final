...
async def bulk_user_action(
    ...
    for user_id in request.user_ids:
        try:
            ...
            logger.error("Invalid ObjectId for user %s: %s", user_id, str(e))
            failed_ids.append(user_id)

    if valid_oids:
        # Find all valid users in one query
        users = await db.users.find({"_id": {"$in": valid_oids}}).to_list(length=None)
        found_oids_str = {str(u["_id"]) for u in users}

        verified_oids = []
        for oid in valid_oids:
            if str(oid) in found_oids_str:
        ...
        if verified_oids:
            try:
                ...
                logger.error("Bulk database action failed: %s", str(e))
                # Add verified oids to failed_ids if the bulk write fails
                failed_ids.extend([str(oid) for oid in verified_oids])
