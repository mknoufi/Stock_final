...
async def validate_record(
    ...
    prefetched_serials: Optional[dict[str, Any]] = None,
    prefetched_rack_locks: Optional[dict[str, Optional[str]]] = None,
    ...
    Args:
        ...
        sync_service: Conflict detection service
        user_id: Current user ID
        prefetched_serials: Optional map of serial_number -> existing document
        prefetched_rack_locks: Optional map of rack_id -> owner_id
    ...
    if record.serial_numbers:
        for serial in record.serial_numbers:
            ...
            if prefetched_serials is not None:
                existing = prefetched_serials.get(serial)
            else:
    ...
    if record.rack_id:
        if prefetched_rack_locks is not None:
            owner = prefetched_rack_locks.get(record.rack_id)
        else:
...
async def sync_batch(
    ...
    try:
        # Bulk fetch data for optimization
        all_serials = set()
        all_rack_ids = set()

        for record in request.records:
            if record.serial_numbers:
                all_serials.update(record.serial_numbers)
            if record.rack_id:
                all_rack_ids.add(record.rack_id)

        # Fetch existing serials
        prefetched_serials = {}
        if all_serials:
            cursor = db.item_serials.find({"serial_number": {"$in": list(all_serials)}})
            async for doc in cursor:
                prefetched_serials[doc["serial_number"]] = doc

        # Fetch rack locks
        prefetched_rack_locks = {}
        if all_rack_ids:
            prefetched_rack_locks = await lock_manager.get_rack_lock_owners_batch(list(all_rack_ids))
        ...
        for record in request.records:
            conflict = await validate_record(
                ...
                sync_service,
                user_id,
                prefetched_serials=prefetched_serials,
                prefetched_rack_locks=prefetched_rack_locks,
            )
            if conflict:
