...
async def sync_batch(
    ...
    try:
        # Batch fetch idempotency operations to prevent N+1 queries
        record_ids = list(
        ...
        if record_ids:
            cursor = db.idempotency_operations.find(
                {"operation_id": {"$in": record_ids}}, {"operation_id": 1, "_id": 0}
            )
            # Use to_list safely
            existing_op_docs = (
                await cursor.to_list(length=None) if hasattr(cursor, "to_list") else []
            )
            existing_op_ids = {
                doc.get("operation_id") for doc in existing_op_docs if doc.get("operation_id")
            }
...
async def _process_legacy_operations(
    ...
    if op_ids:
        ...
        existing_op_docs = await cursor.to_list(length=None) if hasattr(cursor, "to_list") else []
        existing_op_ids = {
            doc.get("operation_id") for doc in existing_op_docs if doc.get("operation_id")
        }
