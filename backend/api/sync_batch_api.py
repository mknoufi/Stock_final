...
async def sync_batch(
    ...
    try:
        # ⚡ Bolt: Batch fetched idempotency operations to fix N+1 query performance bottleneck
        client_record_ids = list({r.client_record_id for r in request.records if r.client_record_id})
        existing_ops = await db.idempotency_operations.find({"operation_id": {"$in": client_record_ids}}).to_list(None) if client_record_ids else []
...
async def _process_legacy_operations(
    ...
    existing_ops = await db.idempotency_operations.find({"operation_id": {"$in": op_ids}}).to_list(None) if op_ids else []
    existing_op_ids = {op["operation_id"] for op in existing_ops}
