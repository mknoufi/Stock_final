...
async def sync_batch(
    ...
    try:
        ...
        # BATCH FETCH OPTIMIZATION: Retrieve idempotency operations outside loop
        req_ids = list({r.client_record_id for r in request.records if r.client_record_id})
        existing_ops = set()
        if req_ids:
            cursor = db.idempotency_operations.find(
                {"operation_id": {"$in": req_ids}}, {"_id": 0, "operation_id": 1}
            )
            existing_ops = {
                doc["operation_id"]
        ...
        for record in request.records:
            ...
            if record.client_record_id in existing_ops:
                ok_records.append(record.client_record_id)
                continue

            # Add to set to prevent duplicate processing within the same batch
            existing_ops.add(record.client_record_id)
...
async def _process_legacy_operations(
    ...
    if op_ids:
        cursor = db.idempotency_operations.find(
            {"operation_id": {"$in": op_ids}}, {"_id": 0, "operation_id": 1}
        )
        existing_ops = {
            doc["operation_id"]
            for doc in await cursor.to_list(length=None)
    ...
    for op in ordered_ops:
        ...
        try:
            # Check idempotency
            if op.id in existing_ops:
                success = True
                message = "Already processed (idempotency)"
            else:
                # Add to set to prevent duplicate processing within the same batch
                existing_ops.add(op.id)
