...
async def _breakdown_by_location(
    ...
    for group in groups:
        ...
        # Get item prices
        item_codes = list({line.get("item_code") for line in lines if line.get("item_code")})  # ⚡ Bolt: Deduplicate item_codes to optimize $in query payload
...
async def _breakdown_by_category(
    ...
    for category, cat_items in category_items.items():
        item_codes = list({item.get("item_code") for item in cat_items if item.get("item_code")})  # ⚡ Bolt: Deduplicate item_codes to optimize $in query payload
...
async def _breakdown_by_session(
    ...
    for session in sessions[:20]:  # Limit to 20 most recent sessions
        ...
        # Get prices
        item_codes = list({line.get("item_code") for line in count_lines if line.get("item_code")})  # ⚡ Bolt: Deduplicate item_codes to optimize $in query payload
        items = await db.erp_items.find({"item_code": {"$in": item_codes}}).to_list(None)
        price_map = {
            item["item_code"]: item.get(valuation_basis, 0) or item.get("mrp", 0) for item in items
        }
