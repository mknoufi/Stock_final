...
async def calculate_dashboard_overview(
    ...
    # Setup value metrics basis
    price_field = valuation_basis if valuation_basis in ["last_cost", "sale_price"] else "last_cost"

    # Use aggregation to calculate ERP totals without loading all items into memory
    erp_totals_pipeline = [
        {
            "$group": {
                "_id": None,
                ...
                "total_stock_value": {
                    "$sum": {
                        "$multiply": [
                            ...
                            {
                                "$cond": {
                                    "if": {"$gt": [{"$ifNull": [f"${price_field}", 0]}, 0]},
                                    "then": {"$ifNull": [f"${price_field}", 0]},
                                    "else": {
                                        "$cond": {
                                            "if": {"$gt": [{"$ifNull": ["$sale_price", 0]}, 0]},
                                            "then": {"$ifNull": ["$sale_price", 0]},
                                            "else": {"$ifNull": ["$mrp", 0]},
                                        }
                                    },
                                }
    ...
    erp_totals_result = await db.erp_items.aggregate(erp_totals_pipeline).to_list(1)

    if erp_totals_result:
        totals = erp_totals_result[0]
        total_stock_qty = totals.get("total_stock_qty", 0)
        items_total = totals.get("items_total", 0)
        total_stock_value = totals.get("total_stock_value", 0)
    else:
        total_stock_qty = 0
        items_total = 0
        total_stock_value = 0
    ...
    # Fetch only counted items to build price map
