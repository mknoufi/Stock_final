...
async def _breakdown_by_location(
    ...
    for group in groups:
        ...
        # Get item prices
        item_codes = list(set(line.get("item_code") for line in lines if line.get("item_code")))
...
async def _breakdown_by_category(
    ...
    for category, cat_items in category_items.items():
        item_codes = list(set(item["item_code"] for item in cat_items if item.get("item_code")))
