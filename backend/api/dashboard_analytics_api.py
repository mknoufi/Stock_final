...
async def _breakdown_by_location(
    ...
    for group in groups:
        ...
        # ⚡ Bolt: Using global_price_map to prevent N+1 query inside loop
...
async def _breakdown_by_category(
    ...
    # ⚡ Bolt: Batch fetch all count lines to prevent N+1 query problem
    ...
    for category, cat_items in category_items.items():
        ...
        for item in cat_items:
            count_lines.extend(lines_by_item.get(item.get("item_code"), []))
...
async def _breakdown_by_session(
    ...
    sessions_to_process = sessions[:20]

    # ⚡ Bolt: Batch fetch all count lines and items to prevent N+1 queries
    ...
    all_items = (
        await db.erp_items.find({"item_code": {"$in": list(all_item_codes)}}).to_list(None)
        if all_item_codes
        else []
    )
    global_price_map = {
    ...
    breakdown = []
    for session in sessions_to_process:  # Limit to 20 most recent sessions
...
async def _breakdown_by_date(db: AsyncIOMotorDatabase, valuation_basis: str) -> List[BreakdownItem]:
    ...
    # ⚡ Bolt: Batch fetch all count lines and items for the 7 days to prevent N+1 queries
    ...
    for days_ago in range(7):
        ...
        # Filter count lines for this date
        count_lines = [
            line
            for line in all_count_lines
