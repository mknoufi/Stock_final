...
def get_zones(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        if sql_connector.test_connection():
            ...
            if zones:
                return zones
            logger.warning("SQL Server connected but returned no zones, falling back to defaults")

        # Offline Fallback
        logger.warning("SQL Server unavailable or empty, returning default offline zones")
        return [
