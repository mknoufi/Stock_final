    ...
    try:
        ...
        if settings_doc:
            ...
            return UserSettingsResponse(
                ...
                data=UserSettings(
                    theme=str(settings_doc.get("theme") or DEFAULT_SETTINGS["theme"]),
                    font_size=str(settings_doc.get("font_size") or DEFAULT_SETTINGS["font_size"]),
                    primary_color=settings_doc.get("primary_color")
                    or DEFAULT_SETTINGS["primary_color"],
                    haptic_enabled=settings_doc.get("haptic_enabled")
                    if settings_doc.get("haptic_enabled") is not None
                    else DEFAULT_SETTINGS["haptic_enabled"],
                    sound_enabled=settings_doc.get("sound_enabled")
                    if settings_doc.get("sound_enabled") is not None
                    else DEFAULT_SETTINGS["sound_enabled"],
                    auto_sync_enabled=settings_doc.get("auto_sync_enabled")
                    if settings_doc.get("auto_sync_enabled") is not None
                    else DEFAULT_SETTINGS["auto_sync_enabled"],
...
async def update_user_settings(
    ...
    try:
        ...
        return UserSettingsResponse(
            ...
            data=UserSettings(
                theme=str(updated_doc.get("theme") or DEFAULT_SETTINGS["theme"]),
                font_size=str(updated_doc.get("font_size") or DEFAULT_SETTINGS["font_size"]),
                primary_color=updated_doc.get("primary_color") or DEFAULT_SETTINGS["primary_color"],
                haptic_enabled=updated_doc.get("haptic_enabled")
                if updated_doc.get("haptic_enabled") is not None
                else DEFAULT_SETTINGS["haptic_enabled"],
                sound_enabled=updated_doc.get("sound_enabled")
                if updated_doc.get("sound_enabled") is not None
                else DEFAULT_SETTINGS["sound_enabled"],
                auto_sync_enabled=updated_doc.get("auto_sync_enabled")
                if updated_doc.get("auto_sync_enabled") is not None
                else DEFAULT_SETTINGS["auto_sync_enabled"],
