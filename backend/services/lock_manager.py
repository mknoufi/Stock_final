...
class LockManager:
    ...
    async def get_rack_lock_owners_batch(self, rack_ids: list[str]) -> dict[str, Optional[str]]:
        """
        Get owners for multiple rack locks in a single request

        Args:
            rack_ids: List of rack identifiers

        Returns:
            Dictionary mapping rack_id to owner (or None)
        """
        if not rack_ids:
            return {}

        # Deduplicate rack IDs
        unique_rack_ids = list(set(rack_ids))
        keys = [f"rack:lock:{rack_id}" for rack_id in unique_rack_ids]

        # Use mget directly on the underlying client for efficiency
        # We don't catch exceptions here so that failures propagate (Fail Closed)
        # instead of incorrectly assuming no locks exist.
        values = await self.redis.client.mget(keys)

        return {
            rack_id: value
            for rack_id, value in zip(unique_rack_ids, values)
        }
