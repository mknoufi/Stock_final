...
class ChangeDetectionSyncService:
    ...
    def _validate_identifier(self, identifier: str) -> bool:
        ...
        if not identifier:
            return False
        # Strict validation: alphanumeric, underscore, dot (for schema.table)
        # Rejects spaces, semicolons, comments, etc.
    ...
    def _get_products_with_changes_query(
        ...
        try:
            ...
            if not self._validate_identifier(table_name):
                return Fail(SyncConfigError(f"Invalid table name in mapping: {table_name}"))

            # Validate and build columns list
            column_parts = []
            for alias, col in mapping["items_columns"].items():
                if not self._validate_identifier(alias):
                    return Fail(SyncConfigError(f"Invalid column alias in mapping: {alias}"))
                if not self._validate_identifier(col):
                    return Fail(SyncConfigError(f"Invalid column name in mapping: {col}"))
                column_parts.append(f"{col} as {alias}")
            ...
            if modified_column:
                if not self._validate_identifier(modified_column):
                    return Fail(SyncConfigError(f"Invalid modified_date_column in mapping: {modified_column}"))
