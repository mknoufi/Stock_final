    ...
    async def get_items_with_fields(
        ...
        try:
                ...
                [
                    {
                        "$group": {
                            "_id": "$item_code",
                            "dynamic_fields_list": {"$push": {"k": "$field_name", "v": "$value"}},
                        }
                    },
                    {"$skip": skip},
                    {"$limit": limit},
                    # Optimization: Join with items collection to avoid N+1 queries
                    {
                        "$lookup": {
                            "from": "items",
                            "localField": "_id",
                    ...
                    {"$unwind": "$item_details"},
                    # Merge dynamic fields into item details
                    {
                        "$project": {
                            "item": "$item_details",
                            "dynamic_fields": {"$arrayToObject": "$dynamic_fields_list"},
                        }
                    },
                ]
            )
            ...
            for result in results:
                item = result["item"]
                # Add dynamic fields to item
                item["dynamic_fields"] = result["dynamic_fields"]
                items.append(item)
