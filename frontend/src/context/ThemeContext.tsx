...
const THEME_METADATA: { key: ThemeKey; name: string; preview: string[] }[] = [
  { key: "light", name: "Light Mode", preview: ["#FFFFFF", "#000000"] },
  { key: "dark", name: "Dark Mode", preview: ["#000000", "#FFFFFF"] },
];
...
const PATTERN_METADATA: { key: PatternType; name: string; icon: string }[] = [
  ...
  { key: "hexagon", name: "Hexagon", icon: "shapes-outline" },
];
...
// Validation helpers
const isValidThemeMode = (v: any): v is ThemeMode =>
  ["light", "dark", "system"].includes(v);

const isValidPattern = (v: any): v is PatternType =>
  PATTERN_METADATA.some(p => p.key === v);

const isValidLayout = (v: any): v is LayoutArrangement =>
  LAYOUT_METADATA.some(l => l.key === v);
...
export const ThemeProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const fontStyle = settings.fontStyle || "modern";
    ...
    return {
      ...
      typography: {
        ...
        fontFamily: {
          ...
          body: fontStyle === "serif" ? "serif" : fontStyle === "mono" ? "monospace" : (theme.typography as any)?.fontFamily?.body,
          heading: fontStyle === "serif" ? "serif" : fontStyle === "mono" ? "monospace" : (theme.typography as any)?.fontFamily?.heading,
        },
        fontSize: {
  ...
  }, [theme, isDark, primaryColor, fontSize, fontStyle]);
  ...
  useEffect(() => {
    const loadSettings = () => {
      try {
        ...
        if (isValidPattern(savedPattern)) {
