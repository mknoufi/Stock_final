...
export const useItemForm = () => {
  ...
  // Auto-sync quantity with serial numbers count
  useEffect(() => {
    if (isSerialEnabled && serialNumbers.length > 0) {
      // For serialized items, quantity is derived from serial count
      const serialCount = serialNumbers.length;
      if (quantity !== String(serialCount)) {
        setQuantity(String(serialCount));
      }
    }
  }, [serialNumbers, isSerialEnabled, quantity]);
