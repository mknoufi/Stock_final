...
export interface Settings {
  ...
  fontSizeValue: number; // Numeric font size (12-24)
  ...
  // Governance
  varianceThreshold: number; // Percentage (e.g., 5 for 5%)
  varianceThresholdEnabled: boolean;
...
export const useSettingsStore = create<SettingsState>((set, get) => ({
  ...
  setSetting: (key, value) => {
    ...
    // Sync to backend if enabled or if it's a key appearance/governance setting
    if (
      newSettings.autoSyncEnabled ||
      ["theme", "fontSizeValue", "fontStyle", "primaryColor", "varianceThreshold"].includes(key)
    ) {
      get()
        .syncToBackend()
        .catch((err) => log.error("Auto-sync failed", { error: err }));
    }
  },
  ...
  syncFromBackend: async () => {
    ...
    try {
      ...
      const updatedSettings: Partial<Settings> = {
        ...
        fontStyle: (backendSettings.font_style || "modern") as Settings["fontStyle"],
  ...
  syncToBackend: async () => {
    ...
    try {
      ...
      const backendPayload: Partial<UserSettings> = {
        ...
        font_style: currentSettings.fontStyle,
