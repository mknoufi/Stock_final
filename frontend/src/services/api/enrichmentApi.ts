...
const ENRICHMENT_BASE = "/api/enrichment";
...
export const enrichItem = async (
  ...
  try {
    const response = await apiClient.post(
      `${ENRICHMENT_BASE}/items/${encodeURIComponent(itemCode)}`,
      enrichmentData,
    );
...
export const getMissingFields = async (
  ...
  try {
    const response = await apiClient.get(
      `${ENRICHMENT_BASE}/items/${encodeURIComponent(itemCode)}/missing-fields`,
    );
    return response.data.data;
  } catch (error: unknown) {
...
export const getEnrichmentHistory = async (
  ...
  try {
    const response = await apiClient.get(
      `${ENRICHMENT_BASE}/items/${encodeURIComponent(itemCode)}/history`,
      {
        params: { limit },
      },
    );
...
export const getIncompleteItems = async (
  ...
  try {
    const response = await apiClient.get(`${ENRICHMENT_BASE}/incomplete-items`, {
...
export const bulkImportEnrichments = async (
  ...
  try {
    const response = await apiClient.post(`${ENRICHMENT_BASE}/bulk-import`, {
      enrichments,
    });
    return response.data.data;
  } catch (error: unknown) {
...
export const checkItemQtyRealtime = async (
  ...
  try {
    const response = await apiClient.get(
      `${ENRICHMENT_BASE}/items/${encodeURIComponent(itemCode)}/check-qty`,
    );
    return response.data.data;
  } catch (error: unknown) {
...
export const recalculateCompleteness = async (): Promise<{
  ...
  try {
    const response = await apiClient.post(
      `${ENRICHMENT_BASE}/recalculate-completeness`,
    );
    return response.data.data;
  } catch (error: unknown) {
