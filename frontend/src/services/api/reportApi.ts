...
export const reportApi = {
  ...
  getReportTypes: async (): Promise<ReportTypeInfo[]> => {
    ...
    return (response.data.report_types || []).map((reportType) => ({
      ...reportType,
      supported_formats: (reportType.supported_formats || []).filter(
        (format): format is ExportFormat => format === "csv" || format === "xlsx",
      ),
    }));
  },
  ...
  getScheduledReports: async (): Promise<ScheduledReport[]> => {
    const response = await api.get<ScheduledReport[]>("/api/exports/schedules");
    return response.data;
  },
  ...
  deleteScheduledReport: async (id: string): Promise<void> => {
    await api.delete(`/api/exports/schedules/${id}`);
  },
  ...
  runScheduledReport: async (id: string): Promise<{ message: string }> => {
    const response = await api.post<{ message: string }>(
      `/api/exports/schedules/${id}/execute`,
    );
    return response.data;
  },
  ...
  > => {
    const response = await api.get("/api/exports/results", { params });
    return response.data;
  },
};
