...
let warnedEnvUnreachable = false;
...
connectionManager.addListener((connection: ConnectionInfo) => {
  if (!connection.backendUrl || connection.errorCode === "NO_BACKEND_FOUND") {
    log.warn("No backend URL available; skipping base URL update", {
  ...
  if (connection.isEnvLocked && !connection.isHealthy) {
    if (!warnedEnvUnreachable) {
      log.warn("Env backend unreachable. Check LAN connectivity.", {
        backendUrl: connection.backendUrl,
        errorCode: connection.errorCode,
      });
      warnedEnvUnreachable = true;
    }
  } else if (connection.isHealthy && warnedEnvUnreachable) {
    warnedEnvUnreachable = false;
  }
});
...
apiClient.interceptors.request.use(
  async (config) => {
    ...
    const isPublic =
      ...
      fullUrl.includes("/metrics/public") ||
