...
export class AsyncStorageService {
  ...
  private debugMode: boolean = typeof __DEV__ !== "undefined" && __DEV__;
  ...
  async setItem<T>(key: string, value: T, options: StorageOptions = {}): Promise<boolean> {
    try {
      ...
      if (this.debugMode && !options.silent) {
        // Log removed to prevent test issues
      }
    ...
    try {
      ...
      try {
        ...
        if (parsed && typeof parsed === "object") {
          ...
          // If it parsed but isn't an object, wrap it
          item = { key, value: parsed as unknown as T };
        }
      } catch {
      ...
      if (!options.ignoreExpiration && item && item.expires && Date.now() > item.expires) {
        if (this.debugMode && !options.silent) {
      ...
      if (this.debugMode && !options.silent && item) {
        // Log removed to prevent test issues
      }

      // Return the value property if it exists (StorageItem format), otherwise the item itself
      return item && "value" in item ? item.value : (item as unknown as T);
    } catch (error) {
        ...
        if (value !== null) {
          try {
            ...
            if (item.expires && Date.now() > item.expires) {
              ...
              result[key] = "value" in item ? item.value : (item as unknown as T);
            }
          } catch {
