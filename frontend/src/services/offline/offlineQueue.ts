...
export async function enqueueMutation(config: AxiosRequestConfig): Promise<QueuedMutation> {
  ...
  // Strip Authorization header — the request interceptor injects fresh
  // tokens on every request, so replayed items will get the current token.
  const sanitizedHeaders = { ...(config.headers as Record<string, string> | undefined) };
...
export function attachOfflineQueueInterceptors(client: AxiosInstance): void {
  ...
  client.interceptors.response.use(
    ...
    async (error) => {
      ...
      // H-6: Exclude endpoints already handled by legacy offlineStorage queue
      // to prevent double-queuing of count lines, sessions, and unknown items
      const legacyPaths = ["/count-lines", "/sessions", "/unknown-items", "/sync/batch"];
      if (cfg.url && legacyPaths.some((p) => cfg.url!.includes(p))) {
        return Promise.reject(error);
      }
