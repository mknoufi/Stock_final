...
export class NotificationService {
  ...
  static async getPushToken(): Promise<string | null> {
    try {
      ...
      errorReporter.report(error, "NotificationService.getPushToken");
      return null;
    }
  }
   ...
   */
  static async registerPushToken(apiClient: any): Promise<boolean> {
    try {
      ...
      log.info("Push token registered", { tokenPrefix: token.substring(0, 20) });
      return true;
    } catch (error) {
      errorReporter.report(error, "NotificationService.registerPushToken");
      return false;
    }
  }
}
