...
class EnvironmentConfig {
  ...
  private async detectBackendUrl(): Promise<string> {
    ...
    const mdnsUrl = `http://${MDNS_HOST}:${mdnsPort}`;

    // 1. mDNS discovery
    try {
      const healthPaths = ["/health/ready", "/health"];
      for (const path of healthPaths) {
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 2000);

        const response = await fetch(`${mdnsUrl}${path}`, {
          method: "GET",
        ...
        if (response.ok) {
          log.info("Found healthy backend via mDNS", mdnsUrl);
          return mdnsUrl;
        }
      }
    } catch (error: any) {
      if (error.name !== "AbortError") {
        log.debug("mDNS backend health check failed", error);
      }
    }
    ...
    // 3. QR scan fallback
    try {
      const qrUrl = await AsyncStorage.getItem(STORAGE_KEYS.BACKEND_QR_URL);
      if (qrUrl) {
        log.info("Using QR fallback backend URL", qrUrl);
        return qrUrl;
      }
    } catch (error) {
    ...
    log.warn("No backend found during discovery");
    return "NO_BACKEND_FOUND";
  }
