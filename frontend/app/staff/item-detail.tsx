...
import { formatBatchMrp } from "@/utils/batchMeta";
...
export default function ItemDetailScreen() {
  ...
  const sameNameVariants = useMemo(() => {
    ...
    return rawVariants.filter((v: any) => {
      ...
      // Do not blindly exclude by barcode as multiple batches can share the same barcode.
      // E.g. Same item_code and barcode, but different batch_no
      if (v.batch_no && item?.batch_no && v.batch_no === item.batch_no) return false;
      // If we don't have batch numbers, we have to show them or risk hiding legitimate batches.
                  ...
                  {sameNameVariants.length === 0 ? (
                    ...
                    <View style={styles.batchList}>
                      {sameNameVariants.map((variant, index) => {
                        ...
                        const batchMrp = formatBatchMrp(
                          variant.mrp ?? variant.mrp_counted ?? variant.price,
                        );
                        const variantKey =
                          variant._id ??
                          [
