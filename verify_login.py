        ...
        if response.status_code == 200:
            ...
            if token:
                ...
                if hb_response.status_code == 200:
                    print(f"      [PASS] Protected endpoint (heartbeat) accessible")
                else:
                    print(
                ...
                if refresh_token:
                    ...
                    if ref_response.status_code == 200:
                        print(f"      [PASS] Refresh token flow successful")
                        new_token = ref_response.json().get("data", {}).get("access_token")
                        if new_token:
                            print(f"      [PASS] New access token obtained")
                        else:
                            print(f"      [FAIL] No access token in refresh response")
                            all_passed = False
                    else:
                        print(
                    ...
                    print(f"      [FAIL] No refresh token returned in login")
            else:
